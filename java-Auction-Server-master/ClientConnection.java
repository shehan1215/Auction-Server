...
public class ClientConnection implements Runnable {
    ...
    private void handle() throws IOException {
        ...
        try {
            ...
            String symbol;
            for (symbol = in.readLine(); !DBaseStock.isExist(symbol); symbol = in.readLine()) {
            ...
            StockItem item = DBaseStock.getStockItem(symbol);
            out.print("\nPlease Wait.......\n");
            out.flush();
            synchronized (item) {
                ...
                try {
                    for (price = in.readLine(); !price.equals("quit")
                            && Double.parseDouble(price) <= currentPrice; price = in.readLine()) {
                        out.print("\nError: Hi " + name
                                + ", The price you entered must be more than the current price of the security. Note that the current price of "
                                + symbol + " is " + currentPrice + "\nPlease re-enter your price to bid : ");
                        out.flush();
                    }
                ...
                out.print("\nOk " + name
                        + ",Your price accepted. Please enter 'confirm' and press enter to comfim bidding. Or enter 'quit' and press enter to quit bidding.\n");
                out.flush();
                ...
                for (String confirm = in.readLine(); !confirm.equals("confirm"); confirm = in.readLine()) {
                    if (confirm.equals("quit"))
                        connectionSocket.close();
                    out.print("\nError input: Hi " + name
                            + ", Enter 'confirm' and press enter to comfim bidding. Or enter 'quit' and press enter to quit bidding.\n");
                    out.flush();
                }
                ...
                DBaseStock.setPrice(symbol, Double.parseDouble(price));

                out.print("\nCongratulations " + name
                        + ",Your bid saved successfully. Thank you for using Stock Exchange - Auction Server.\n");
                out.flush();
            }
            connectionSocket.close();
